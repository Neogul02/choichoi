          ...
          <div className="bg-[#f9f9f9] rounded-xl p-4">
            <div className="flex justify-between items-center mb-2.5">
              ...
              <div className="flex items-center gap-2">
                <button className="border-none w-7 h-7 rounded-md bg-[#e9ecef] cursor-pointer" onClick={() => moveCalendarMonth(-1)}>&lt;</button>
                <strong className="min-w-[110px] text-center text-sm">{monthYearLabel}</strong>
                <button className="border-none w-7 h-7 rounded-md bg-[#e9ecef] cursor-pointer" onClick={() => moveCalendarMonth(1)}>&gt;</button>
              </div>
            </div>
            <div className="flex justify-between bg-white border border-[#eee] rounded-lg px-3 py-2.5 mb-3 text-[13px]">
              <span>월 주문 {calendarSales.totalOrders}건</span>
              <strong className="text-primary-700 font-bold">월 매출 ₩{Number(calendarSales.monthTotal || 0).toLocaleString('ko-KR')}</strong>
            </div>
            {isCalendarLoading ? (
              ...
              <div className="grid grid-cols-7 gap-1.5">
                {['일', '월', '화', '수', '목', '금', '토'].map((dayName) => (
                  <div key={dayName} className="text-center text-xs text-[#666] font-semibold py-1">{dayName}</div>
                ))}
                {Array.from({ length: firstDay }).map((_, idx) => <div key={`blank-${idx}`} className="min-h-[68px]" />)}
                {Array.from({ length: daysInMonth }).map((_, idx) => {
                  ...
                  return (
                    <div key={dateKey} className="min-h-[68px] border border-[#ececec] rounded-lg bg-white p-1.5">
                      <div className="text-xs font-bold mb-1.5">{day}</div>
                      <div className="text-[11px] text-[#333] leading-[1.3]">{dayRevenue > 0 ? `₩${dayRevenue.toLocaleString('ko-KR')}` : '-'}</div>
                    </div>
                  );
                })}
              </div>
