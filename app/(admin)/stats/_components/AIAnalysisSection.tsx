...
import type { TodaysSales, MenuSalesItem, OrderRecordWithItems } from '@/types/api';
...
export default function AIAnalysisSection({ summary, todayOrders, menuBreakdown }: Props) {
  ...
  const handleAnalyze = async () => {
    ...
    HOURS.forEach((h) => { hourMap[h] = { revenue: 0, orderCount: 0 }; });
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-sm font-semibold shadow-sm hover:opacity-90 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
      ...
      {analysis && (
        <div className="rounded-xl bg-gradient-to-br from-[#f8f7ff] to-[#f0f4ff] border border-[#e2dcff] p-4">
          ...
          <p className="mt-3 text-[11px] text-[#bbb] text-right">총 매출 ₩{formatPrice(summary.totalRevenue)} · {summary.totalOrders}건</p>
        </div>
      )}
    </div>
  );
