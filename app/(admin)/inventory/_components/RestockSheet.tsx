    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl p-6 shadow-2xl"
        ...
        >
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={onClose} className="text-[#999] text-xl leading-none">✕</button>
          </div>

          {/* 박스 수 */}
          <div className="mb-4">
            <label className="text-xs font-bold text-[#888] uppercase tracking-wide block mb-2">
              입고 수량 ({ingredient.container_unit})
            </label>
            <div className="flex items-center gap-3 mb-2">
              <button
                onClick={() => setSealedDelta((v) => Math.max(1, v - 1))}
                className="w-10 h-10 rounded-full border border-[#e0e0e0] text-xl font-bold text-[#444] hover:bg-[#f5f6f7] transition-colors"
              >−</button>
              <span className="text-3xl font-black tabular-nums w-12 text-center">{sealedDelta}</span>
              <button
                ...
                <button
                  ...
                  className={`text-xs font-bold px-3 py-1 rounded-full border transition-colors ${
                    sealedDelta === n
                      ...
                      : 'bg-white text-[#666] border-[#e0e0e0] hover:border-primary-700'
                  }`}
                >
                  {n}
                </button>
          ...
          <div className="mb-5">
            ...
            <input
              ...
              placeholder="예: 마켓컬리 구매, 유통기한 6/30"
              className="w-full border border-[#e0e0e0] rounded-xl px-3 py-2.5 text-sm outline-none focus:border-primary-700 transition-colors"
              style={{ userSelect: 'text', WebkitUserSelect: 'text' }}
            />
          </div>

          {/* 프리뷰 */}
          <div className="bg-[#f5f6f7] rounded-xl px-4 py-3 mb-5 text-sm text-[#555]">
            입고 후 재고:{' '}
            <span className="font-bold text-[#161616]">
              {formatQty(afterTotal, ingredient)}
            </span>
          ...
          <button
            ...
            disabled={saving}
            className="w-full bg-primary-700 hover:bg-primary-800 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-colors text-sm"
          >
