...
interface Props {
  ...
  initialSales: TodaysSales | null
  initialOrders: OrderRecordWithItems[] | null
}

export default function PosPageClient({ initialPopupId, initialMenu, initialSales, initialOrders }: Props) {
  ...
  const salesQuery = useQuery<TodaysSales>({
    ...
    staleTime: 60_000,
    // 서버 프리페치 데이터는 같은 popupId일 때만 유효
    ...(initialSales != null && popupId === initialPopupId ? { initialData: initialSales } : {}),
  })
  ...
  const menuQuery = useQuery<MenuItem[]>({
    ...
    ...(initialMenu != null ? { initialData: initialMenu } : {}),
  })
