...
export default function PasswordGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      try {
        const { data } = await axios.post(VALIDATE_API_PATH, { token: storedToken });
        if (data.valid) {
          setIsAuthed(true);
        } else {
          clearStaffStorage();
        ...
        <div className="w-full max-w-[360px]">
          ...
          <div className="flex rounded-xl overflow-hidden border border-hairline mb-5 bg-canvas shadow-level-1">
            {(['staff', 'admin'] as Tab[]).map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => { setTab(t); setError(''); setPassword(''); setAdminPassword(''); }}
          ...
          <div className="text-center mb-5">
            ...
            <p className="m-0 text-ink-muted text-sm">
              {tab === 'staff' ? '운영 화면 접근을 위해 정보를 입력해주세요.' : '관리자 계정으로 로그인해주세요.'}
            </p>
          </div>

          {tab === 'staff' ? (
            <form className="bg-canvas rounded-xl p-5 shadow-level-1 border border-hairline" onSubmit={onSubmitStaff}>
              ...
              <input type="text" className={inputClass} value={name} onChange={(e) => setName(e.target.value)} placeholder="이름" autoFocus={popupEvents.length === 1} autoComplete="name" />
              <input type="password" className={inputClass} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="비밀번호" autoComplete="current-password" />
              {error && <div className="text-[#b42318] text-[13px] mb-3">{error}</div>}
              <button type="submit" className="w-full border-none rounded-lg px-3 py-2.5 text-[14px] font-bold bg-primary-700 text-white cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" disabled={isSubmitting}>
                {isSubmitting ? '확인 중...' : '입장하기'}
              </button>
            </form>
          ) : (
            <form className="bg-canvas rounded-xl p-5 shadow-level-1 border border-hairline" onSubmit={onSubmitAdmin}>
              <div className="relative mb-3">
                <select
                  className={`${inputClass} mb-0 appearance-none pr-8 cursor-pointer ${!selectedPopupId ? 'text-ink-faint' : 'text-ink'} ${popupEvents.length === 1 ? 'opacity-70 cursor-default' : ''}`}
                  value={selectedPopupId}
              ...
              <input type="text" className={inputClass} value={name} onChange={(e) => setName(e.target.value)} placeholder="이름" autoComplete="name" />
              <input type="email" className={inputClass} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="이메일" autoFocus autoComplete="email" />
              <input type="password" className={inputClass} value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="비밀번호" autoComplete="current-password" />
              {error && <div className="text-[#b42318] text-[13px] mb-3">{error}</div>}
              <button type="submit" className="w-full border-none rounded-lg px-3 py-2.5 text-[14px] font-bold bg-primary-700 text-white cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" disabled={isSubmitting}>
